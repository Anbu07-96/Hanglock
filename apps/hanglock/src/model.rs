    ...
    pub fn on_command(&mut self, cmd: Command) -> Vec<Action> {
        ...
        match cmd {
            Command::ToggleVisible => {
                self.set_visible(!self.settings.overlay.enabled);
                out.push(Action::Save);
            }
            Command::ToggleTopmost => {
                self.settings.overlay.topmost = !self.settings.overlay.topmost;
                out.push(Action::Save);
            }
            Command::ToggleSeconds => {
                self.settings.face.seconds = !self.settings.face.seconds;
                out.push(Action::Save);
            }
            Command::Toggle12Hour => {
                self.settings.face.hour12 = !self.settings.face.hour12;
                out.push(Action::Save);
            }
            ...
            Command::ResetPosition => out.extend(self.reset_place()),
            Command::SetClickThrough(mode) => out.extend(self.set_mouse(mode)),
            Command::SetMonitor(index) => out.extend(self.set_monitor(index)),
            Command::SetAnchor(want) => {
                if want != Anchor::from_overlay(&self.settings.overlay) {
...
impl Model {
    ...
    /// Showing and hiding is one decision with two consequences, and three commands need it: the
    ...
    /// not change which display it is on: a clock moved to the monitor on the left is still on that
    /// monitor, and the user meant that.
    fn reset_place(&mut self) -> Vec<Action> {
        let o = &mut self.settings.overlay;
        o.anchor_ratio = Anchor::DEFAULT.ratio;
        o.anchor_drop = Anchor::DEFAULT.drop;
        o.hang = 150.0;
        o.scale = 1.0;
        let was_hidden = !o.enabled;
        o.enabled = true;
        if was_hidden {
            self.state = State::Settled;
        }
        self.notice = None;
        vec![Action::Relayout, Action::Save]
    }

    /// The mouse mode, with the one refusal the app is allowed to make on the user's behalf: a fully
    /// click-through clock cannot be clicked to be put right, so it is not entered while the tray —
    /// the way back — is not there.
    fn set_mouse(&mut self, mode: ClickThrough) -> Vec<Action> {
        if mode == ClickThrough::Always && !self.tray_ok {
            self.notice = Some(TRAY_REQUIRED);
            return Vec::new();
        }
        self.notice = None;
        if self.settings.overlay.click_through == mode {
            return Vec::new();
        }
        self.settings.overlay.click_through = mode;
        vec![Action::Save]
    }

    /// Which display to hang from. A clock that was hidden comes back, because a switch that appeared
    /// to do nothing reads as a failure rather than as a memory of an earlier choice.
    fn set_monitor(&mut self, index: u32) -> Vec<Action> {
        if self.settings.overlay.monitor_index == index {
            return Vec::new();
        }
        self.settings.overlay.monitor_index = index;
        self.set_visible(true);
        vec![Action::Relayout, Action::Save]
    }
}
...
pub fn diag(settings: &Settings, monitors: &[Monitor], found: &dyn std::fmt::Display) -> String {
    ...
    out.push_str(&diag_where(&m, settings, &chosen, frame, p.clipped));
    out.push_str(&diag_habits(&m, settings));
    out.push_str(&diag_costs(&mut m, settings));
    out
}

/// Where the clock ends up, in both coordinate systems and in the file's own units — three ways of
/// naming one position, printed together because the disagreement between them is the bug report.
fn diag_where(
    m: &Model,
    settings: &Settings,
    chosen: &Monitor,
...
/// What the document says the clock should be, line by line, including the two claims that belong to
/// the OS rather than to the file (always-on-top, autostart) and so are printed as *wanted*.
fn diag_habits(m: &Model, settings: &Settings) -> String {
    let o = &settings.overlay;
    let mut out = String::new();
    out.push_str(&format!(
        "  clock           {}\n",
...
/// The frame the settings actually produce, and what drawing it costs. `diag` paints once, here, so the
/// canvas numbers describe the clock the settings describe rather than an empty rig.
fn diag_costs(m: &mut Model, settings: &Settings) -> String {
    m.paint();
    let mut out = String::new();
    out.push_str(&format!(
...
mod tests {
    ...
    fn alt_at_the_press_reaims_rather_than_grabbing() {
        ...
        let c = m.rope.card_centre();
        m.on_input(Input::Press { at: c, alt: true });
        assert!(
            m.is_repositioning(),
    ...
    fn a_reanchor_moves_the_window_under_the_card_instead_of_carrying_it() {
        ...
        let a = m.rope.anchor;
        let before = m.rope.card_centre();
        m.on_input(Input::Press { at: a, alt: true });
    ...
    fn a_dragged_position_survives_the_settings_round_trip() {
        ...
        assert_eq!(
            back, m.settings,
            "the document did not survive its own writer"
        );
    ...
    fn the_three_click_through_modes_answer_the_mouse_differently() {
        ...
        assert!(
            !regions(&m).contains(p),
            "empty pixels must stay the desktop's"
        );
        ...
        assert!(
            regions(&m).contains(p),
            "a normal window takes its own corners"
        );
    ...
    fn startup_adopts_a_login_entry_the_file_does_not_know_about() {
        ...
        assert_eq!(
            again,
            vec![Action::Relayout],
            "a quiet startup is a quiet startup"
        );
    }
