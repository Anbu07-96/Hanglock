...
**Process finding.** The committed `docs/previews/01-settled-light.png` is the *flattering* frame: the
card level, the cord vertical, the anchor centred. Even there the muddy mass of shadow, the beaded
5 px cable with its 8 px halo, the marker-weight digits and the blue `PM` are all visible — so the
look was reviewable in Phase 1 and was reviewed as a demo of the painter rather than as a design.
Meanwhile the frame the app actually paints first (`hanglock_ref.py render`'s `00_rest`, 17° off
vertical) was never committed at all: the worst view was the one nobody saw. Both are in
`previews/phase-2.5/` now, and the concept command prints the frame numbers next to each image so a
mismatch between a number and a picture cannot hide again.
