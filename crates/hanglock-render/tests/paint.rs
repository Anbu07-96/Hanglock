...
fn scene_at(theta: f64, centre: Vec2, text: FaceText) -> Scene {
    let mut rope = Rope::new(RopeConfig::default(), CardSpec::default(), Posture::PLATE, Vec2::new(280.0, 14.0), 1.0);
...
fn face(s: &str) -> FaceText {
    let c = Civil { year: 2026, month: 9, day: 15, hour: 22, minute: 42, second: 7 };
    let _ = s;
    format(&c, &FaceOptions { hour12: true, seconds: false, meridiem: true })
}
...
fn every_untouched_pixel_is_exactly_transparent() {
    ...
    for (x, y) in [(0u32, 0u32), (w - 1, 0), (0, h - 1), (w - 1, h - 1), (5, 380), (690, 300)] {
        assert_eq!(cv.alpha_at(x, y), 0, "pixel ({x},{y}) must be fully transparent");
    }
}
...
fn the_plate_is_opaque_where_it_is_drawn() {
    ...
    assert!(below > 0 && below < a, "no soft shadow below the plate ({below} vs {a})");
}
...
fn a_tilted_plate_still_covers_its_own_centre() {
    for th in [-0.35, -0.1, 0.0, 0.1, 0.35] {
        ...
        paint(&sc, &mut cv, &Theme::default());
        assert!(cv.alpha_at(350, 200) > 150, "rotation {th} left a hole in the plate");
    }
}
...
fn the_drawn_cord_stops_where_the_plate_covers_it() {
    ...
    assert!(sc.drawn_cord_len() + 1 < sc.node_count as usize, "cord was not trimmed at the plate");
    paint(&sc, &mut cv, &Theme::default());
    assert!(!cv.px.iter().all(|&b| b == 0), "an all-black frame is not a painted frame");
}
...
fn painting_is_deterministic() {
    ...
    assert_eq!(a.px, b.px, "two paints of one scene must agree, or the golden images mean nothing");
}
...
fn a_glyph_is_wider_than_nothing_and_the_face_has_every_digit() {
    ...
    for ch in "0123456789:APM".chars() {
        ...
        assert!(!g.lines.is_empty() || !g.dots.is_empty(), "{ch} draws nothing");
    }
    assert!(glyph('\u{1F600}').is_none(), "an unplanned character must be skipped, not drawn as a box");
}
