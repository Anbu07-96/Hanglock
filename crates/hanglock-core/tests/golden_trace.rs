...
fn solver_matches_the_reference_model() {
    ...
    let anchor: Vec<f64> = expect_prefix("anchor")
        .iter()
    ...
    let mut rope = Rope::new(
        ...
        CardSpec::default(),
        Posture::PLATE,
        Vec2::new(anchor[0], anchor[1]),
        1.0,
    );
    rope.host = Some(Rect::new(0.0, 0.0, 560.0, 300.0));
    assert_eq!(
        rope.cfg.segments, seg,
        "segment count drifted from the reference"
    );
    assert!(
        (rope.hang - hang).abs() < TOL,
        "hang length drifted: {} vs {hang}",
        rope.hang
    );
    ...
    for i in 0..frames {
        ...
        let line = lines
            .next()
            .expect("trace shorter than its header promised");
        ...
        assert!(
            (rope.att.theta - th).abs() < 1e-7,
            "frame {i}: attitude diverged"
        );
        let asleep = toks[4 + (seg + 1) * 2] == "1";
        assert_eq!(
            rope.sleeping, asleep,
            "frame {i}: sleep state diverged (the idle contract)"
        );
        let stretch: f64 = toks[5 + (seg + 1) * 2].parse().unwrap();
        assert!(
            (rope.max_stretch() - stretch).abs() < 1e-7,
            "frame {i}: stretch diverged"
        );
    }
