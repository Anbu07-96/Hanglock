...
fn a_partial_document_keeps_everything_it_does_not_mention() {
    ...
    assert!(s.overlay.topmost, "unrelated default must survive a partial file");
    assert_eq!(s.face.posture, PostureKind::Plate);
    assert!(w.iter().all(|x| !x.contains("ignored")), "a known key must not be reported unknown: {w:?}");
}
...
fn an_old_document_loads_into_a_new_build() {
    ...
    assert_eq!(s.overlay.margin, 16.0, "a field the old file never had takes its default");
}
...
fn a_round_trip_loses_nothing() {
    ...
    assert!(warnings.is_empty(), "a file we wrote ourselves must not warn: {warnings:?}");
    assert_eq!(s, back);
}
...
fn writing_twice_produces_the_same_bytes() {
    ...
    assert_eq!(s.to_toml(), s.to_toml(), "a settings diff in a bug report has to mean something");
}

#[test]
fn posture_and_click_through_names_round_trip() {
    for p in [PostureKind::Natural, PostureKind::Plate, PostureKind::Mounted, PostureKind::Locked] {
    ...
    assert_eq!(PostureKind::parse("Plate"), None, "keys are lower-case on purpose; a typo is reported");
}

#[test]
fn sanitize_is_idempotent_and_total() {
    let mut s = Settings { schema: 99, ..Settings::default() };
    s.overlay.hang = f64::NAN;
    s.overlay.anchor_ratio = -1.0;
    s.sanitize();
    assert!(s.overlay.hang.is_finite(), "NaN must not survive the boundary");
